...
GitHub Pages では `main` ブランチの `/docs` フォルダを公開元にしてください。
公開用ファイルは次のコマンドで `public/` から同期します。

```bash
npm run build:github-pages
```

GitHub の Settings → Pages で Source を `Deploy from a branch`、Branch を `main`、
Folder を `/docs` にすると、`https://<user>.github.io/<repository>/` で公開されます。
